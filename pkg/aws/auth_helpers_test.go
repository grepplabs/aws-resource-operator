...
func TestAWSGetCredentials_shouldError(t *testing.T) {
	resetEnv := unsetEnv(t)
	defer resetEnv()
	setEnvCredentialsLocation("not_existing_credentials", t)
	cfg := Config{}
...
// TestAWSGetCredentials_shouldIAM is designed to test the scenario of running operator
...
func TestAWSGetCredentials_shouldBeShared(t *testing.T) {
	file, err := ioutil.TempFile(os.TempDir(), "aws_resource_operator_aws_cred")
	if err != nil {
...
// overwrite shared credentials location for testing EC2RoleProvider and when credential files
// in the default location exists
func setEnvCredentialsLocation(s string, t *testing.T) {
	if err := os.Setenv("AWS_SHARED_CREDENTIALS_FILE", s); err != nil {
		t.Fatalf("Error setting env var AWS_SHARED_CREDENTIALS_FLE: %s", err)
	}
}
...
func awsMetadataApiMock(endpoints []*endpoint) func() {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		logger.Printf("[DEBUG] Mocker server received request to %q", r.RequestURI)
