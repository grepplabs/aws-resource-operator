...
func GetAccountIDAndPartitionFromEC2Metadata() (string, string, error) {
	log.Info("[DEBUG] Trying to get account information via EC2 Metadata")
...
func GetAccountIDAndPartitionFromIAMGetUser(iamconn *iam.IAM) (string, string, error) {
	log.Info("[DEBUG] Trying to get account information via iam:GetUser")
...
func GetAccountIDAndPartitionFromIAMListRoles(iamconn *iam.IAM) (string, string, error) {
	log.Info("[DEBUG] Trying to get account information via iam:ListRoles")
...
func GetAccountIDAndPartitionFromSTSGetCallerIdentity(stsconn *sts.STS) (string, string, error) {
	log.Info("[DEBUG] Trying to get account information via sts:GetCallerIdentity")
...
func GetCredentials(c *Config) (*awsCredentials.Credentials, error) {
	...
	if userTimeout != "" {
		...
		if err == nil {
			if newTimeout.Nanoseconds() > 0 {
				...
				logInfof("[WARN] Non-positive value of %s (%s) is meaningless, ignoring", userTimeoutEnvVar, newTimeout.String())
			}
		} else {
			logInfof("[WARN] Error converting %s to time.Duration: %s", userTimeoutEnvVar, err)
		}
	}

	logInfof("Setting AWS metadata API timeout to %s", client.Timeout.String())
	cfg := &aws.Config{
		HTTPClient: client,
	}
	...
	if uri := os.Getenv("AWS_CONTAINER_CREDENTIALS_RELATIVE_URI"); len(uri) > 0 {
		...
		log.Info("ECS container credentials detected, RemoteCredProvider added to auth chain")
	}
	...
	if !c.SkipMetadataApiCheck {
		...
		if metadataClient.Available() {
			...
			log.Info("AWS EC2 instance detected via default metadata API endpoint, EC2RoleProvider added to the auth chain")
		} else {
			if usedEndpoint == "" {
				usedEndpoint = "default location"
			}
			logInfof("Ignoring AWS metadata API endpoint at %s "+
	...
	logInfof("Attempting to AssumeRole %s (SessionName: %q, ExternalID: %q, Policy: %q)",
	...
	if err != nil {
		if awsErr, ok := err.(awserr.Error); ok && awsErr.Code() == "NoCredentialProviders" {
			return nil, fmt.Errorf("the role %s cannot be assumed", c.AssumeRoleARN)
		}
...
func setOptionalEndpoint(cfg *aws.Config) string {
	...
	if endpoint != "" {
		logInfof("Setting custom metadata endpoint: %q", endpoint)
